...
public final class PolyMeshGpuRenderer {
    ...
    public static void renderAfterSolid(RenderPass sharedPass) {
        ...
        // 【A1 · 渲染目标覆盖防御 —— 26.3 起不再需要】
        // 26.2 这里挡的是 RenderSystem.outputColorTextureOverride：那是个全局字段，
        // 别的 mod 可以在任何时刻把输出重定向到未知离屏 target，带着它画 = 枪画丢。
        // 26.3 删掉了那两个 override，输出目标改成 RenderPass 显式参数、由开 pass 的
        // 调用方自己持有 —— 没有任何人能在 TACZ 背后偷换输出目标，这个危害在结构上
        // 就不存在了，所以守卫一并删掉（留注释避免后来者照着 26.2 再加回来）。
        if (drawnThisFrame) {
    ...
    public static void renderWorldAfterSolid(RenderPass sharedPass) {
        ...
        // 【A1】同上：26.3 没有 outputColorTextureOverride 这个全局重定向，
        // 「带 override 的 executeSolid」这种组合不可能再出现，守卫删掉。
    ...
    private static long drawViaRenderTypeCore(List<DrawEntry> draws, boolean handPass, RenderPass sharedPass) {
        ...
        // 【纹理必须在 render pass 之外解析】与 drawList 同一条踩坑（见其注释）：
        // 26.3 的 drawFromBuffer 要显式传 pass，于是 RenderType.prepare() ——
        // 它内部会把纹理解析进 PreparedRenderType.textures —— 落到了 pass 里面，
        // 而«pass 开着不许发其他命令»会让懒加载的 writeToTexture 直接抛异常。
        // 先在这里把每张贴图预热一遍，pass 里的 prepare() 就只会命中已加载的纹理。
        for (Identifier texture : byTexture.keySet()) {
            if (resolveTextureView(texture) == null) {
                resolveTextureView(MissingTextureAtlasSprite.getLocation());
            }
        }

        // 【26.3 迁移】26.2 的 PreparedRenderType#drawFromBuffer(vb, ib, type, 0, 0, n)
        // 自己在内部开 pass、画到当时的全局输出目标（可被 outputColorTextureOverride
        // 改写）。26.3 把签名换成 (StagedVertexBuffer.ExecuteInfo, RenderPass)：
        // 六个参数打包进 ExecuteInfo 并多出一个 topology，输出目标改由调用方开 pass 决定。
    ...
    /**
     * {@link #drawViaRenderTypeCore} 的内层：在已开好的 pass 里逐纹理组、逐骨骼绘制。
     *
     * <p>拆出来只为让 pass 的 try-with-resources 不再套进三层循环，绘制语义
     * （压栈 → prepare() → drawFromBuffer → 弹栈 的先后）与 26.2 逐字一致。</p>
     */
    private static long drawGroupsInPass(Map<Identifier, List<DrawEntry>> byTexture,
                                         boolean handPass,
                                         Matrix4fStack mvStack,
                                         RenderPass pass) {
        long totalIndices = 0;
        for (Map.Entry<Identifier, List<DrawEntry>> group : byTexture.entrySet()) {
            ...
            for (DrawEntry entry : group.getValue()) {
                ...
                try {
                    ...
                    prepared.drawFromBuffer(new StagedVertexBuffer.ExecuteInfo(
                            entry.bone().vertexBuffer, indexBuffer, indices.type(),
                            0, 0, entry.bone().indexCount, PrimitiveTopology.QUADS), pass);
                } finally {
                    mvStack.popMatrix();
                }
