 ...
 *
 * <p>The lightweight entry points are implementation details rather than API, so this class uses
 * reflection and only probes a viewer that is installed: JEI Fabric's
 * {@code mezz.jei.fabric.events.JeiLifecycleEvents.AFTER_RECIPES_UPDATED} (an event whose invoker is
 * {@link Runnable}) and REI's {@code me.shedaniel.rei.RoughlyEnoughItemsCoreClient#reloadPlugins} with
 * two nullable arguments. Where an installed viewer has no such entry point (for example JEI on
 * NeoForge), one normal client resource reload is used as a safe fallback for the connection; it is
 * never retriggered by the fallback itself.</p>
 */
public final class RecipeViewerReloadBridge {
    ...
    /** Runs at the end of each client tick, once the client has a level and player. */
    public static void tick(Minecraft client) {
        ...
        // A viewer without a lightweight hook gets exactly one resource-reload fallback per
        // connection. Resource reload does not enqueue this bridge, preventing a reload loop.
        resourceFallbackUsed = true;
        GunMod.LOGGER.info("[TACZ Recipe Viewer] Viewer reload hook unavailable on {}; falling back once to a client resource reload.",
                Platform.INSTANCE.loaderName());
        try {
    ...
    /** JEI Fabric invokes its plugin-rebuild listener through this Runnable event. */
    ...
    /** REI 26.2 reloads all plugin stages through reloadPlugins(MutableLong, ReloadStage). */
    ...
    private static boolean hasJei() {
        return Platform.INSTANCE.isModLoaded("jei");
    }

    private static boolean hasRei() {
        return Platform.INSTANCE.isModLoaded("roughlyenoughitems") || Platform.INSTANCE.isModLoaded("rei");
    }
}
