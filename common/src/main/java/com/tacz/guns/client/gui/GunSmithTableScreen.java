...
import com.tacz.guns.mixin.accessor.GuiGraphicsExtractorAccessor;
...
public class GunSmithTableScreen extends AbstractContainerScreen<GunSmithTableMenu> {
        ...
        this.addRenderableWidget(new TaczImageButton(leftPos + 112, topPos + 164, 18, 18, 149, 211, 18, TEXTURE, b -> {
            if (this.selectedRecipe != null) {
                ...
                if (StringUtils.isNotBlank(url) && minecraft != null) {
                    // 26.3: ConfirmLinkScreen and Blaze3D.openUri both take a URI. Gun pack metadata is
                    // downloadable content, so parse through vanilla to reject a bad or disallowed scheme.
                    final URI uri;
                    try {
    ...
    private void renderIngredient(GuiGraphicsExtractor gui) {
        ...
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 2; j++) {
                ...
                TaczIngredient ingredient = smithTableIngredient.getIngredient();

                ItemStack[] items = ingredient == null
                        ? new ItemStack[0]
                        : ingredient.getDisplayStacks(SlotDisplayContext.fromLevel(Minecraft.getInstance().level)).toArray(ItemStack[]::new);
    ...
    private void renderLeftModel(GuiGraphicsExtractor graphics, GunSmithTableRecipe recipe) {
        ...
        // GuiGraphicsExtractor's scissor stack is private and cannot be read, so there is no intersection with an
        // outer scissor: this screen never draws the preview inside another scissor, so the preview box is the area.
        ScreenRectangle scissor = new ScreenRectangle(startX, startY, width, height);

        ((GuiGraphicsExtractorAccessor) graphics).tacz$getGuiRenderState().addPicturesInPictureState(new GunPreviewRenderState(
                renderState,
