...
public final class ScopePipRenderer {
    ...
    private static RenderPipeline compositePipeline() {
        if (compositePipeline == null) {
            BindGroupLayout maskLayout = BindGroupLayout.builder().withUniform(MASK_SAMPLER, UniformType.COMBINED_IMAGE_SAMPLER).build();
    ...
    public static void renderScopeView(Minecraft mc,
        ...
        try {
            ...
            try {
                // 【26.3 签名变更】render() 去掉了 deltaTracker 与 viewRotationMatrix
                // 两个参数（前者本就只在内部取 partialTick，后者现在从 CameraRenderState
                // 自己读），末尾多出一个 boolean。
                //
                // 那个新 boolean 在 vanilla 调用点传的是
                // {@code !appliedPostEffects.isEmpty()}（GameRenderer 字节码实读），
                // 在 render() 内部有两处用途：与 frameHasAlwaysOnTopGizmos() 一起决定
                // 是否在帧图里另建 always_on_top_depth，以及传给 addMainPass ——
                // 也就是「后处理链在跑，置顶 gizmo 需要自己的深度附件才能活过后处理」。
                // 镜内这一遍画进 PIP 自己的 target、不跑任何后处理链，也不要置顶
                // gizmo（与上面方块线框传 false 同一个理由），所以传 false。
