 ...
 * 此时以 F3 右上角显存为准。堆数据直接读内存与 GC MXBean，无需权限。</p>
 *
...
public final class ScopePipResourceProbe {
    ...
    public static void onScopePassEnd() {
        ...
        long heapUsedMiB = ManagementFactory.getMemoryMXBean().getHeapMemoryUsage().getUsed() / (1024L * 1024L);
    ...
    public static void beginFrame() {
        ...
        try {
            ...
            long heapUsedMiB = heap.getUsed() / (1024L * 1024L);
            long heapMaxMiB = heap.getMax() / (1024L * 1024L);
            long gcCount = 0;
            long gcTimeMs = 0;
