...
public class EntityBulletRenderer extends EntityRenderer<EntityKineticBullet, EntityBulletRenderer.BulletRenderState> {
        ...
        getModel().ifPresent(model -> {
            ...
            poseStack.pushPose();
            {
                ...
                if (isFirstPerson) {
                    ...
                    //      poseStack.rotate(new Matrix4f(viewRotationMatrix).invert())  // 基座 R(q)，view→world
    ...
    @Override
    public boolean shouldRender(EntityKineticBullet bullet, Frustum camera, double pCamX, double pCamY, double pCamZ, float partialTick) {
