...
import com.tacz.guns.crafting.ingredient.NbtTaczIngredient;
...
public class GunSmithTableIngredient {
    ...
    @Nullable
    private TaczIngredient ingredient;
    ...
    public GunSmithTableIngredient(TaczIngredient ingredient, int count) {
    ...
    /**
     * @return 解析后的 {@link TaczIngredient}；若原始 JSON 至今仍无法解析（例如 tag 真的不存在）则返回 {@code null}。
     *         调用方必须判空——这正是「配方整条消失」与「材料格空着但配方还在」的区别。
     */
    @Nullable
    public TaczIngredient getIngredient() {
        if (this.ingredient == null && this.rawItem != null) {
            JsonElement raw = this.rawItem;
            try {
                this.ingredient = TaczIngredient.CODEC.parse(
     ...
     * {"item": ..., "nbt": {...}}    ->  {"type": "tacz:nbt", "items": [...], "nbt": {...}, "partial": true}
     * </pre>
     *
     * <h2>为什么必须转换</h2>
    ...
    private static JsonElement normalizeLegacy(JsonElement raw) {
        if (raw.isJsonObject()) {
            ...
            // 【带 type 的自定义 Ingredient】
            //
            // Legacy Forge and upstream TACZ NBT ingredient types are rewritten into the one object
            // form read by NbtTaczIngredient.CODEC (details in normalizeCustomIngredient):
            //
            //   Forge : {"type":"forge:partial_nbt","item":"tacz:modern_kinetic_gun",
            //            "nbt":{"GunId":"hamster:coltm1892"}}
            //   TACZ  : {"type":"tacz:nbt","items":["tacz:modern_kinetic_gun"],
            //            "nbt":{"GunId":"hamster:coltm1892"},"partial":true}
            //
            // 语义完全保持不变：仍然是「必须是带该 NBT 的那件物品」，
            // 不放宽成「任意 TACZ 枪械」—— 这正是之前刻意不转换的理由，
            // 现在既然有了等价实现，就能在不改变游戏行为的前提下真正支持它。
            //
            // Only the types in SUPPORTED_CUSTOM_INGREDIENTS are rewritten; any other type is returned
            // unchanged so the CODEC reports it, rather than us guessing an equivalent that hides the real problem.
            if (obj.has("type")) {
            ...
            if (item != null && item.isJsonPrimitive() && item.getAsJsonPrimitive().isString()) {
                ...
                if (obj.has("nbt") && obj.get("nbt").isJsonObject()) {
                    ...
                    out.add("type", new JsonPrimitive(NbtTaczIngredient.TYPE.toString()));
                    JsonArray items = new JsonArray(1);
                    items.add(new JsonPrimitive(item.getAsString()));
                    out.add("items", items);
                    out.add("nbt", obj.get("nbt"));
                    out.add("partial", new JsonPrimitive(true));
    ...
    /**
     * Custom ingredient types that {@link #normalizeCustomIngredient} rewrites into the {@link NbtTaczIngredient} form.
     *
     * <p>不在此集合中的 type 不做任何改写 —— 宁可让它明确失败，
    ...
    /**
     * Rewrites a supported typed custom ingredient into the canonical {@code tacz:nbt} object.
     * 见 {@link #normalizeLegacy} 中的说明。
     *
    ...
    private static JsonElement normalizeCustomIngredient(JsonObject obj) {
        ...
        JsonObject out = new JsonObject();
        // All supported types share the tacz:nbt discriminator; the match mode travels in "partial" (see below).
        out.add("type", new JsonPrimitive(NbtTaczIngredient.TYPE.toString()));
        for (java.util.Map.Entry<String, JsonElement> entry : obj.entrySet()) {
            ...
            // 单数 item(字符串) -> 复数 items(列表)，对齐 NbtTaczIngredient.CODEC 声明的字段。
        ...
        // The Forge types imply the match mode (their serializers had no "partial" field):
        // forge:partial_nbt is a subset match, forge:nbt an exact match. tacz:nbt keeps its own
        // optional "partial" (default false), already copied above.
        if (!NbtTaczIngredient.TYPE.toString().equals(type)) {
            out.add("partial", new JsonPrimitive("forge:partial_nbt".equals(type)));
        }
        return out;
    }

    /** 供必须拿到非空值的场合（如网络编码）使用。 */
    public TaczIngredient getIngredientOrThrow() {
        TaczIngredient resolved = this.getIngredient();
        if (resolved == null) {
