...
public final class GetJarResources {
    ...
    public static void copyModDirectory(Class<?> resourceClass, String srcPath, Path root, String path) {
        ...
        try {
            ...
            } else if (resourceClass == GunMod.class) {
                // NeoForge's class loader cannot look up directories, so walk TACZ's own mod files instead.
                exportModFolderIfChanged(GunMod.MOD_ID, srcPath, root, path);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private static void exportModFolderIfChanged(String modId, String srcPath, Path root, String path) throws IOException {
        String directory = srcPath.startsWith("/") ? srcPath.substring(1) : srcPath;
        Map<String, IoSupplier<InputStream>> files = new TreeMap<>();
        if (!Platform.INSTANCE.visitModFiles(modId, directory, files::put) || files.isEmpty()) {
            return;
        }
        // Loaders do not all expose file sizes or dates here; the mod version and the file list stand in for them.
        String stateKey = getExportStateKey(GunMod.class, srcPath, path);
        String sourceFingerprint = Md5Utils.md5Hex((Platform.INSTANCE.getModVersion(modId).orElse("") + "\n"
                + String.join("\n", files.keySet())).getBytes(StandardCharsets.UTF_8));
        ExportStateFile stateFile = readExportState(root);
        Path targetPath = root.resolve(path);
        if (Files.isDirectory(targetPath) && sourceFingerprint.equals(stateFile.entries.get(stateKey))) {
        ...
        for (Map.Entry<String, IoSupplier<InputStream>> file : files.entrySet()) {
            Path target = targetPath.resolve(file.getKey());
            Files.createDirectories(target.getParent());
            try (InputStream inputStream = file.getValue().get()) {
    ...
    private static void backupFiles(Path targetPath) throws IOException {
        ...
        Path resourcePacksPath = Platform.INSTANCE.getGameDir().resolve("tacz_backup");
