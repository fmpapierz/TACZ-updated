...
public class CombatProperties {
    ...
    private void playAttackSound(MeleeAction action, Vec3 origin) {
        ...
        //
        // The display lookup only exists on the client: dedicated servers have no display classes at all, which is
        // why it sits in the nested Client class below, loaded only when this line is reached on the client.
        //
        ...
        Client.playAttackSound(entity, action, origin);
    }

    /**
     * Client-only half of {@link #playAttackSound}. Dedicated servers load {@code CombatProperties} but have none of
     * the display classes, so this nested class is only loaded once a sound is played on the client.
     */
    private static final class Client {
        private Client() {
        }

        static void playAttackSound(Player entity, MeleeAction action, Vec3 origin) {
            me.xjqsh.lrtactical.api.client.LrTacticalClientAPI.getMeleeDisplay(entity.getMainHandItem())
