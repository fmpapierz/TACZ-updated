...
public class MeleeItemRenderer extends AnimateGeoItemRenderer<CustomBedrockModel, BaseAnimationStateContext> {
    ...
    public Identifier getTextureLocation(ItemStack stack) {
        return LrTacticalClientAPI.getMeleeDisplay(stack).map(MeleeDisplayInstance::getTexture).orElse(null);
    }

    @Override
    @Nullable
    public LuaAnimationStateMachine<BaseAnimationStateContext> getStateMachine(ItemStack stack) {
        return LrTacticalClientAPI.getMeleeDisplay(stack).map(MeleeDisplayInstance::getStateMachine).orElse(null);
    }

    @Override
    @Nullable
    public CustomBedrockModel getModel(ItemStack stack) {
        return LrTacticalClientAPI.getMeleeDisplay(stack).map(MeleeDisplayInstance::getModel).orElse(null);
    }
    ...
    public void renderByItem(ItemStack stack, ItemDisplayContext ctx, PoseStack poseStack,
        ...
        MeleeDisplayInstance display = LrTacticalClientAPI.getMeleeDisplay(stack).orElse(null);
        if (display == null) {
