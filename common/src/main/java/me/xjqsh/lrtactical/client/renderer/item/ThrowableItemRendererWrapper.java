...
public class ThrowableItemRendererWrapper
    ...
    public Identifier getTextureLocation(ItemStack stack) {
        return LrTacticalClientAPI.getThrowableDisplay(stack).map(ThrowableDisplayInstance::getTexture).orElse(null);
    }

    @Override
    @Nullable
    public LuaAnimationStateMachine<ThrowableAnimationStateContext> getStateMachine(ItemStack stack) {
        return LrTacticalClientAPI.getThrowableDisplay(stack).map(ThrowableDisplayInstance::getStateMachine).orElse(null);
    }

    @Override
    @Nullable
    public CustomBedrockModel getModel(ItemStack stack) {
        return LrTacticalClientAPI.getThrowableDisplay(stack).map(ThrowableDisplayInstance::getModel).orElse(null);
    }
    ...
    public void renderByItem(ItemStack stack, ItemDisplayContext ctx, PoseStack poseStack,
        ...
        ThrowableDisplayInstance display = LrTacticalClientAPI.getThrowableDisplay(stack).orElse(null);
        if (display == null) {
