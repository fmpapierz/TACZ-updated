 ...
 * 这正是上一轮记下的教训。
 *
 * <p>In the multiloader build this runs from {@code TaczCommonEvents#shouldCancelAttackEntity}, which each loader
 * connects to its attack event; the injection points above are Fabric's. NeoForge fires its event at the head of
 * {@code Player#attack}, after the client has already sent the vanilla attack packet, but the server then cancels
 * that attack the same way, so no vanilla damage is dealt either.
 */
