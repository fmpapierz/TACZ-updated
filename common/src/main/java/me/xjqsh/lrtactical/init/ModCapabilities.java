 ...
 *
 * <p>The multiloader build keeps these maps: they behave the same on Fabric, Quilt, NeoForge and Forge, so the
 * data needs neither NeoForge attachments nor Forge capabilities.
 *
...
public final class ModCapabilities {
    ...
    public static void init() {
        ...
        // The respawn hook is TaczCommonEvents#onPlayerRespawn, which every loader calls.
    }
     ...
     *
     * <p>Called from {@code TaczCommonEvents#onPlayerRespawn}, which only has the new player. It carries the old
     * player's network id, so removing it drops the stale entry; should the id ever stop being reused, the new player
     * gets a fresh entry anyway and the old one is left for the weak map to discard.
     */
    public static void onRespawn(net.minecraft.server.level.ServerPlayer player) {
        synchronized (SERVER_COOL_DOWNS) {
            SERVER_COOL_DOWNS.remove(player);
        }
        synchronized (SERVER_COMBAT) {
            SERVER_COMBAT.remove(player);
        }
    }

    /**
     * 客户端侧的同一问题 —— called every client tick from {@code TaczClientEvents#onClientTickStart}.
     *
     ...
     * 每 tick 一次引用比较，开销可忽略。
     *
     * <p>The parameter is a {@link Player} rather than a {@code LocalPlayer} so that this common class never
     * refers to client-only classes.
     *
